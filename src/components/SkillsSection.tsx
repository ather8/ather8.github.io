...
const SkillsSection = () => {
  ...
  return (
    <section id="skills" className="py-20 sm:py-24 px-4 sm:px-6 bg-card/50" aria-labelledby="skills-heading">
      <div ref={ref} className={`max-w-5xl mx-auto transition-all duration-700 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
        ...
        <div className="grid sm:grid-cols-2 gap-5 sm:gap-8">
          {skillCategories.map((cat, i) => (
            <div
              key={cat.title}
              className={`group bg-card border border-border rounded-xl p-5 sm:p-6 hover:border-primary/50 hover:shadow-lg hover:shadow-primary/5 transition-all duration-500 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}
